define(
    ...
    function ($, Component, placeOrderAction, selectPaymentMethodAction, customer, checkoutData, additionalValidators) {
        ...
        $('.errP').text('');
        var promo='';
        /*alert(window.checkoutConfig.payment.grezpay.hide_promo_field);
        alert($.trim(window.checkoutConfig.payment.grezpay.promoCode));
        alert($.trim(window.checkoutConfig.payment.grezpay.promoCode).length);
        alert(window.checkoutConfig.payment.grezpay.promoCode);*/
        if(window.checkoutConfig.payment.grezpay.hide_promo_field=='1'){
            if(window.checkoutConfig.payment.grezpay.promo_code_local_validation=='1' && ($.trim(window.checkoutConfig.payment.grezpay.promoCode).length=='0' || $.trim(window.checkoutConfig.payment.grezpay.promoCode)=='')){
        ...
        $(document).on('click','.promoApplyBtn',function(){
            // alert(window.checkoutConfig.payment.grezpay.promoCode);
            if(window.checkoutConfig.payment.grezpay.promo_code_local_validation=='1'){
                if($.trim($('.promoCodeField').val())==''){
                    $('.errP').text('Please enter your promo code.');
                }else{
                    var promoCode=window.checkoutConfig.payment.grezpay.promoCode;
        ...
        return Component.extend({
            defaults: {
                template: 'Grezpaypg_Grezpay/payment/grezpaypg'
            },
            ...
            afterPlaceOrder: function () {
                if(promo==''){
                    $.mage.redirect(window.checkoutConfig.payment.grezpay.redirectUrl);
                }else{
                    $.mage.redirect(window.checkoutConfig.payment.grezpay.redirectUrl+"?promo="+promo);
                }
            }
        });
    }

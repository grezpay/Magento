...
use Grezpaypg\Grezpay\Helper\Data as DataHelper;

class Grezpay extends \Magento\Payment\Model\Method\AbstractMethod
{
    const CODE = 'grezpay';
    ...
    protected $_formBlockType = 'Grezpaypg\Grezpay\Block\Form\Grezpay';
    protected $_infoBlockType = 'Grezpaypg\Grezpay\Block\Info\Grezpay';
    ...
    public function __construct(
        ...
        \Grezpaypg\Grezpay\Helper\Data $helper
    ) {
        $this->helper = $helper;
        parent::__construct(
            $context,
    ...
    public function buildGrezpayRequest($order)
    {
        $callBackUrl=$this->urlBuilder->getUrl('grezpay/Standard/Response', ['_secure' => true]);
    ...
    $post_group_data['TXNTYPE']='SALE';
      $post_group_data['HASH']="$hash";
        if(isset($order->grezpayPromoCode)){
            $params['PROMO_CAMP_ID']=$order->grezpayPromoCode;
        }
